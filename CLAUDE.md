...
GitHub repo: `davidbloomq/personal-site` (public). Pushes to `main` auto-deploy via `.github/workflows/deploy.yml` → GitHub Pages. Custom domain `david-bloom.com` via Cloudflare DNS (A records + CNAME, DNS-only/gray cloud). Nothing is live until it's on `main` — work on a feature branch is invisible on david-bloom.com, and David has asked "why don't I see it on the site" when work sat on a branch, so say explicitly when something is not yet deployed.
