...
- **Lightweight names**: commenters get a simple, seamless option to add a name to each comment — not a profile or account, just an optional per-comment name field. If no name is given, don't display one (anonymous).
